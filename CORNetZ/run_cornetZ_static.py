...
def run_net(args, test): 
    ...
    if(args['v'] != 'None'):
        f_info = 'CORNetZ_' + str(args['v']) + 'initial_ratio=' + str(args['v_ratio']) + 'num_epochs=' + str(args['num_epochs']) + 'n_e_c=' + str(args['n_e_c']) + 'n_e_v=' + str(args['n_e_v']) + '_' + str(r)
        df = pd.DataFrame(np.zeros((args['num_epochs'], 9)), columns=['train_fine_acc', 'train_coarse_acc', 'test_fine_acc','test_coarse_acc', 'cif_cost_train', 'cif_cost_test', 'v_cost','ratio', 'time'])
    else:
